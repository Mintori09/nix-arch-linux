...
let
  cfg = config.programs.lose_law;

  loseLaw = pkgs.writeShellApplication {
    name = "lose_law";
  ...
  bashSnippet = ''
    case $- in
      *i*) command -v lose_law >/dev/null 2>&1 && lose_law ;;
    esac
  '';

  zshSnippet = ''
    [[ -o interactive ]] && command -v lose_law >/dev/null 2>&1 && lose_law
  '';
in
{
  options.programs.lose_law = {
    enable = lib.mkEnableOption "run lose_law when opening bash or zsh";
  };

  config = lib.mkIf cfg.enable {
    home.packages = [ loseLaw ];

    programs.bash = {
      enable = lib.mkDefault true;
      initExtra = lib.mkAfter bashSnippet;
    };

    programs.zsh = {
      enable = lib.mkDefault true;
      initExtra = lib.mkAfter zshSnippet;
    };
  };
}
