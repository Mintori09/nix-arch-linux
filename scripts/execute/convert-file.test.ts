...
Deno.test(
  ...
  () => {
    ...
    assertStrictEq(result.exitCode, 0);
    assertStrictEq(result.stdout.includes("- mhtml:png"), true);
    assertStrictEq(result.stdout.includes("- mhtml:jpg"), true);
    assertStrictEq(result.stdout.includes("- mhtml:webp"), true);
  },
);
...
Deno.test(
  ...
  async () => {
    ...
    try {
      ...
      assertStrictEq(result.exitCode, 0);
      assertStrictEq(result.stdout.includes("chromium --headless"), true);
      assertStrictEq(result.stdout.includes("--remote-debugging-port=0"), true);
      assertStrictEq(result.stdout.includes("--window-size=1280,720"), false);
      assertStrictEq(result.stdout.includes(`--screenshot=${output}`), false);
      assertStrictEq(result.stdout.includes(`file://${input}`), true);
    } finally {
