...
- **Config** (`src/config.ts`): Env vars `DAEMON_HOST` (default `127.0.0.1`), `AI_BRIDGE_PORT` (default `58721`), and `AI_BRIDGE_PROMPTS_DIR` (default `~/.config/ai-bridge/prompts/`).
- **Prompt Templates**: Daemon serves `.md` files from `AI_BRIDGE_PROMPTS_DIR` via `GET /prompts` + `GET /prompts/:name`. Userscript shows them as checkboxes in panel (auto-fill) or via `!` autocomplete in input.
