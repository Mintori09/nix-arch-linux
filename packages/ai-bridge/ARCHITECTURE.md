...
### 2.6 Prompt Templates — Browser panel + `!` autocomplete

```mermaid
sequenceDiagram
    participant UserScript as UserScript (Gemini)
    participant Daemon as Daemon (58721)
    participant FS as Prompt Dir (FS)

    Note over UserScript: page load
    UserScript->>Daemon: GET /prompts
    Daemon->>FS: readdir *.md
    FS-->>Daemon: [summarize.md, translate.md]
    Daemon-->>UserScript: [{name, title, preview}]
    UserScript->>UserScript: render checkbox list in panel

    Note over UserScript: auto-fill defaults
    UserScript->>Daemon: GET /prompts/summarize.md
    Daemon->>FS: readFile
    FS-->>Daemon: "Summarize: {{content}}"
    Daemon-->>UserScript: {name, title, content}
    UserScript->>UserScript: combine template with input → fillInput()

    Note over UserScript: ! autocomplete
    UserScript->>UserScript: user types "!tra" in input
    UserScript->>UserScript: filter prompts → ["translate.md"]
    UserScript->>UserScript: user selects → replace "!tra" with content at cursor
```
...
- `AI_BRIDGE_PROMPTS_DIR` (env var): path to prompt templates folder (`.md` files). Default: `~/.config/ai-bridge/prompts/`.
...
| `GET`    | `/prompts`       | —                      | `[{name, title, preview}]`                     |
| `GET`    | `/prompts/:name` | —                      | `{name, title, content}`                       |
...
Daemon serves `.md` files from `AI_BRIDGE_PROMPTS_DIR` to the userscript. Two endpoints:

- `GET /prompts` — list all `.md` files (name, title from filename, 80-char preview)
- `GET /prompts/:name` — read file content, return as `{name, title, content}`

Path traversal protection: reject names with `..` or `/`.

**Panel checkbox list** (`core/prompts.js`):

- Fetch `GET /prompts` on init → render checkbox list in sidebar
- **Auto-fill on load**: checked defaults (stored in `GM_getValue('ai_bridge_default_prompts', [])`) have their content fetched and combined with input text
- **Toggle**: un/check → re-combine immediately (no Apply button)
- **Combine logic**: each template with `{{content}}` → replace; without → prepend content. Joined by `\n\n---\n\n`
- **Default config**: ⚙ icon → overlay with all prompts → save selected to `GM_setValue`

**`!` autocomplete** (`core/autocomplete.js`):

- Detect `!` followed by word chars at cursor in contenteditable input
- Floating popup with fuzzy-filtered prompt list
- `↑/↓` navigate, `Enter` select, `Escape` close
- Replace `!query` with template content at cursor via Range API
- Prompts list cached from daemon (fetched once on load)
...
│   │       ├── focus.ts
│   │       ├── clipboard.ts
│   │       └── prompts.ts            # GET /prompts, GET /prompts/:name
...
│   │   │   ├── prompts.js         # Prompt template panel + default config + combine logic
│   │   │   └── autocomplete.js    # ! trigger popup + fuzzy search in input
│   │   ├── platforms/
...
| `AI_BRIDGE_PROMPTS_DIR` missing  | `/prompts` returns `[]`. Daemon continues normally                                                                        |
| Prompt file not found            | `/prompts/:name` returns 404                                                                                              |
| Prompt file path traversal       | Daemon rejects names with `..` or `/`, returns 400                                                                        |
